...
class ControlConnection(object):
    ...
    def _try_connect(self, host):
        ...
        try:
            ...
            self.refresh_node_list_and_token_map(connection, preloaded_results=shared_results)
            self.refresh_schema(connection, preloaded_results=shared_results)
        except Exception:
            connection.close()
            raise
    ...
    def refresh_schema(self, keyspace=None, table=None, preloaded_results=None):
        try:
            if self._connection:
                self._refresh_schema(self._connection, keyspace, table, preloaded_results=preloaded_results)
        except ReferenceError:
    ...
    def refresh_node_list_and_token_map(self, preloaded_results=None):
        try:
            if self._connection:
                self._refresh_node_list_and_token_map(self._connection, preloaded_results=preloaded_results)
        except ReferenceError:
            pass  # our weak reference to the Cluster is no good
        except Exception:
